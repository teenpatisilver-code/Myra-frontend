...
export default function DrinkCard({ drink }: DrinkCardProps) {
  ...
  return (
    ...
    >
      <Link href={`/menu/${drink.id}`} data-testid={`card-drink-${drink.id}`}>
        <div className="glass-card rounded-xl overflow-hidden border border-border hover:border-primary/30 transition-all duration-300 group cursor-pointer h-full flex flex-col">
          ...
          <div className="p-3 flex flex-col flex-1 gap-2">
            ...
            <div className="mt-auto flex items-center justify-between pt-1">
              <div>
                <span className="font-bold text-foreground text-base">Rs {Math.round(discountedPrice)}</span>
                {hasDiscount && (
                  <span className="text-xs text-muted-foreground line-through ml-1">Rs {Math.round(drink.price)}</span>
                )}
              </div>
              <Button
                size="sm"
                ...
                data-testid={`button-add-to-cart-${drink.id}`}
              >
