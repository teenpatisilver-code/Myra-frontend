    ...
    return (
      <div className="w-full h-48 md:h-64 rounded-2xl bg-gradient-to-br from-primary/20 via-muted to-secondary/20 flex items-center justify-center border border-primary/20">
        <div className="text-center">
          <h2 className="text-3xl md:text-5xl font-serif font-bold text-primary neon-text">
            MYRA DRINKS
          </h2>
          <p className="text-muted-foreground mt-2">
            Premium Beverages in Kathmandu
          </p>
          <Link href="/menu">
...
function SectionHeader({ title, href }: { title: string; href: string }) {
  return (
    <div className="flex items-center justify-between mb-4">
      <h2 className="text-lg font-bold text-foreground font-serif tracking-wide">
        {title}
      </h2>
      <Link
        href={href}
...
export default function HomePage() {
  ...
  const { data: featuredDrinks, isLoading: featuredLoading } =
    ...
    <Layout>
      <div className="py-4 space-y-8">
        ...
        <section>
          ...
          <DrinkGrid
            drinks={featuredDrinks ?? undefined}
