...
import {
  useListDrinks,
  useListCategories,
  getListDrinksQueryKey,
} from "@/api";

export default function MenuPage() {
  const search = useSearch();
  const params = new URLSearchParams(search);
  const initialCategory = params.get("categoryId") ? Number(params.get("categoryId")) : null;
  ...
  const { data: drinks, isLoading: drinksLoading } = useListDrinks(
    selectedCategory ? { categoryId: selectedCategory } : undefined,
    {
      query: {
        queryKey: getListDrinksQueryKey(selectedCategory ? { categoryId: selectedCategory } : undefined),
      },
    }
  );

  const filtered = searchQuery
    ? drinks?.filter((d) => d.name.toLowerCase().includes(searchQuery.toLowerCase()) || d.description?.toLowerCase().includes(searchQuery.toLowerCase()))
    : drinks;
    ...
    <Layout>
      <div className="py-4 space-y-5">
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
          ...
          {catLoading && Array.from({ length: 5 }).map((_, i) => <Skeleton key={i} className="h-8 w-24 rounded-full flex-shrink-0" />)}
          {categories?.map((cat) => (
            <button
              key={cat.id}
