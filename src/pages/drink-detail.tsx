...
export default function DrinkDetailPage() {
  ...
  useEffect(() => {
    if (!id) return;
    supabase
      .from("menu_items")         // ✅ use menu_items not drinks
      .select("*, categories(name)")
      .eq("id", id)
      .single()
      .then(({ data, error }) => {
        if (error || !data) {
          // fallback try drinks table
          supabase.from("drinks").select("*, categories(name)").eq("id", id).single()
            .then(({ data: d2, error: e2 }) => {
              if (e2 || !d2) { setLocation("/menu"); return; }
              setDrink(d2);
              setLoading(false);
              fetchAIBenefits(d2);
            });
  ...
  const fetchAIBenefits = async (drink: any) => {
    ...
    try {
      ...
      const prompt = `You are a health and nutrition expert. For the drink "${drink.name}"${ingredients ? ` made with: ${ingredients}` : ""}, provide a 2-3 sentence summary of its health benefits. Keep it friendly, positive, and concise.`;
  ...
  const handleAddToCart = () => {
    ...
    // ✅ Always use UUID string — never integer
  ...
  return (
    <Layout>
      {/* ✅ pb-32 so content never hidden behind fixed button */}
      <div className="py-4 space-y-5 pb-32">
        ...
        >
          {drink.image_url && !imgError ? (
            <img
              src={drink.image_url} alt={drink.name}
        ...
        <motion.div
          ...
          transition={{ delay: 0.1 }} className="space-y-2"
        >
          ...
          >
            ...
            <div className="flex flex-wrap gap-2">
              {ingredients.map((ing: string) => (
                <span key={ing} className="px-3 py-1 rounded-full bg-primary/10 text-primary text-xs font-medium border border-primary/20">
                  {ing}
                </span>
              ))}
            </div>
