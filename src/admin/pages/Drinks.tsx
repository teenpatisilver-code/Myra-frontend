...
export default function Drinks() {
  ...
  const aiGenerate = async (type: 'description' | 'price') => {
    ...
    try {
      ...
      if (type === 'description') setForm(f => ({ ...f, description: text.trim() }))
      else if (!isNaN(parseFloat(text.trim()))) setForm(f => ({ ...f, price: text.trim() }))
    } catch {
      alert('AI failed.')
    }
    setAiLoading(false)
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="sm:col-span-2">
              <div className="flex items-center justify-between mb-1">
                ...
                <button
                  onClick={() => aiGenerate('description')}
                  disabled={aiLoading || !form.name}
            ...
            <div className="sm:col-span-2">
              <div className="flex items-center justify-between mb-1">
                ...
                <span className="text-xs text-gray-500">comma separated, e.g. mango, milk, honey</span>
              </div>
              <input
                value={form.ingredients}
                onChange={e => setForm(f => ({ ...f, ingredients: e.target.value }))}
                placeholder="e.g. orange, ginger, lemon, honey"
                className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white placeholder:text-gray-500 focus:outline-none focus:border-amber-500"
              />
            </div>
            ...
            <div className="p-4">
              ...
              <div className="flex items-center justify-between">
                <div className="flex gap-2">
                  ...
                  {d.is_featured && (
                    <span className="px-2 py-0.5 rounded-full text-xs bg-amber-500/20 text-amber-400">Featured</span>
                  )}
                </div>
                <div className="flex gap-2">
