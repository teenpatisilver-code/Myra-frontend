...
import { RefreshCw, Bell, X, MapPin, Phone, Package } from 'lucide-react'
...
export default function Orders() {
  ...
  const fetchOrders = async () => {
    let query = supabase
      .from('orders')
      .select('*, profiles(email, phone), order_items(*)')
  ...
  return (
    <div>
        ...
        <div className="fixed top-4 right-4 z-50 bg-amber-500 text-black rounded-xl p-4 shadow-2xl max-w-sm animate-bounce">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="text-xs">{newOrderAlert.profiles?.email}</p>
              <p className="text-xs font-bold">Rs {newOrderAlert.total_amount?.toFixed(2)}</p>
              <p className="text-xs capitalize">{newOrderAlert.order_type?.replace('_', ' ')}</p>
            </div>
          ...
          <div className="bg-gray-900 rounded-2xl border border-gray-700 w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="bg-gray-800 rounded-xl p-4 space-y-2">
                ...
                <p className="text-sm text-white">{selectedOrder.profiles?.email || 'Guest'}</p>
                {(selectedOrder.profiles?.phone || selectedOrder.phone_number) && (
              ...
              {/* Order Type & Address */}
              <div className="bg-gray-800 rounded-xl p-4 space-y-2">
                <h4 className="text-xs text-gray-400 font-semibold uppercase tracking-wider">Delivery Info</h4>
                <p className="text-sm text-white capitalize">
                  {selectedOrder.order_type?.replace(/_/g, ' ') || 'Pickup'}
                </p>
                  ...
                  selectedOrder.order_items.map((item: any) => (
                    <div key={item.id} className="flex justify-between text-sm">
                      ...
                      <span className="text-amber-400">
                        Rs {Math.round((item.unit_price || 0) * item.quantity)}
                      </span>
                    </div>
                  ))
                ) : (
                  <p className="text-gray-500 text-xs">{selectedOrder.notes}</p>
                )}
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {['all', 'pending', 'confirmed', 'preparing', 'on_the_way', 'ready', 'delivered', 'completed', 'cancelled'].map(f => (
          <button
            key={f}
            onClick={() => setFilter(f)}
            ...
            {orders.map((order) => (
              ...
              >
                ...
                <td className="p-4 text-gray-400 text-xs">
                  {order.profiles?.phone || order.phone_number || '—'}
                </td>
