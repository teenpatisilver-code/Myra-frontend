...
const empty = { title: '', subtitle: '', image_url: '', link_url: '', cta_text: 'Order Now', is_active: true, sort_order: 0 }
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))}
                className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white" placeholder="e.g. Summer Special" />
            </div>
            <div>
              ...
              <input type="text" value={form.subtitle} onChange={e => setForm(f => ({ ...f, subtitle: e.target.value }))}
                className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white" placeholder="e.g. 20% off all smoothies" />
            </div>
            <div>
              ...
              {form.image_url && <img src={form.image_url} className="mt-2 w-full h-24 rounded-lg object-cover" />}
            </div>
            <div>
              ...
              <input type="text" value={form.link_url} onChange={e => setForm(f => ({ ...f, link_url: e.target.value }))}
                className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white" placeholder="e.g. /menu" />
            </div>
            <div>
              ...
              <input type="text" value={form.cta_text} onChange={e => setForm(f => ({ ...f, cta_text: e.target.value }))}
                className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white" placeholder="e.g. Order Now" />
            </div>
            <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        {banners.map(b => (
          <div key={b.id} className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
            {b.image_url && <img src={b.image_url} className="w-full h-32 object-cover" />}
            <div className="p-4">
              <div className="flex items-start justify-between mb-1">
                <h4 className="font-semibold text-white">{b.title}</h4>
                <span className={`px-2 py-0.5 rounded-full text-xs ${b.is_active ? 'bg-green-500/20 text-green-400' : 'bg-gray-500/20 text-gray-400'}`}>
                  {b.is_active ? 'Active' : 'Hidden'}
                </span>
              </div>
              {b.subtitle && <p className="text-xs text-gray-400 mb-3">{b.subtitle}</p>}
              <div className="flex gap-2">
                <button onClick={() => edit(b)} className="flex-1 bg-gray-800 hover:bg-gray-700 px-3 py-1.5 rounded-lg text-xs text-gray-300">Edit</button>
                ...
                <button onClick={() => del(b.id)} className="p-1.5 bg-red-500/10 hover:bg-red-500/20 rounded-lg text-red-400">
                  <Trash2 size={14} />
                </button>
              </div>
            </div>
